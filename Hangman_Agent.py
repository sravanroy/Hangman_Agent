# words come from here: https://projecteuler.net/project/resources/p042_words.txt
...
def choose_word(words_in):
    valid_word = False
    rand_word = ""
    while not valid_word:
        rand_word = random.choice(words_in)
        if len(rand_word) > 3:
            valid_word = True
    return rand_word


def get_letter(alphabet, letters_guessed_in):
    is_letter = False
    letter = ""
    while not is_letter:
        letter = input("Enter a letter: ").upper()
        if (letter in alphabet) and (letter not in letters_guessed_in):
            is_letter = True
        elif letter in letters_guessed_in:
            cool_print("You've guessed that before. (>_<)")
        else:
            cool_print("That's not a letter. (>_<)")
    return letter


def check_guess(letter_in, word_in):
    indices = [idx for idx, char in enumerate(word_in) if letter_in == char]
    return indices


def show_word(word_in, visible_indices_in):
    cool_print_word = ""
    for i in range(len(word_in)):
        if i in visible_indices_in:
            cool_print_word += word_in[i] + " "
        else:
            cool_print_word += "_ "
    return cool_print_word, "_" not in cool_print_word


def show_picture(fails_in, word_in):
    """
    shows a picture for a given number of failed attempts
    """
    if fails_in == 0:
        cool_print("________      ")
        cool_print("|      |      ")
        ...
        cool_print("|     / \     ")
        cool_print("|             ")
        cool_print("The noose tightens around your neck.")
        cool_print("As you pass your last moments on Earth,")
        cool_print("you realize that the word was {}".format(word_in))
        cool_print("GAME OVER!")


if __name__ == "__main__":
    cool_print("Welcome to HANGMAN!!!")
    words = ["A", "ABILITY", "ABLE", "ABOUT", "ABOVE", "ABSENCE", "ABSOLUTELY", "ACADEMIC", "ACCEPT", "ACCESS",
             ...
             "YOUR", "YOURSELF", "YOUTH"]  # it took me a long time to type that in (@_@)
    ...
    while True:
        ...
        guessed = False
        # cool_print(word)
        display_word = show_word(word, visible_indexes)
        show_picture(fails, word)
        cool_print("Word: " + display_word[0] + "     " + "Letters guessed: " + letters_guessed)
        while (fails < 7) and (guessed is False):
            guess = get_letter("ABCDEFGHIJKLMNOPQRSTUVWXYZ", letters_guessed)
            letters_guessed += guess
            correct_indices = check_guess(guess, word)
            if correct_indices is True:
                guessed = True
            elif len(correct_indices) == 0:
                cool_print("Sorry, that letter isn't in the word.")
                fails += 1
            else:
                cool_print("Good guess!")
            ...
            guessed = display_word[1]

            if guessed:
                cool_print("You WIN!!!! (%__%)")

        cool_print("------------------------")
        cool_print("Let's play another game!")
